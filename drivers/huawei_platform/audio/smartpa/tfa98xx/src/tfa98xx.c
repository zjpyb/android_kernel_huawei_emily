	...
	int len, unsigned char value[])
{
	...
	reg_buf = (unsigned char *)kmalloc(sizeof(reg), GFP_KERNEL);
	if (!reg_buf) {
		return -ENOMEM;;
	}
...
static void tfa98xx_adsp_send_calib_values(void)
{
	...
	tfa98xx_sec = get_tfa_handle(CHAN_PRI_R);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		if (ret == Tfa98xx_Error_Not_Supported) {
			...
			reschedule = true;
			/* the dsp communication error occurred starting PA, we should do hardware reset first. */
			if (Tfa98xx_Error_RpcBase <= ret) {
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		/* an error occurred stoping PA, we should do hardware reset first. */
...
{
	struct tfa98xx *tfa98xx_pri = NULL, *tfa98xx_sec = NULL;
	struct tfa98xx *tfa98xx_pri_sub = NULL, *tfa98xx_sec_sub = NULL;
	int selection = 0;
	int ret = 0, value = 0;
	...
	tfa98xx_pri = get_tfa_handle(CHAN_PRI_L);
	tfa98xx_sec = get_tfa_handle(CHAN_PRI_R);
	tfa98xx_pri_sub = get_tfa_handle(CHAN_SEC_L);
	tfa98xx_sec_sub = get_tfa_handle(CHAN_SEC_R);
	pr_info("tfa98xx_pri=%p  tfa98xx_sec=%p  tfa98xx_sec_sub=%p  tfa98xx_pri_sub=%p\n",
		tfa98xx_pri, tfa98xx_sec, tfa98xx_sec_sub, tfa98xx_pri_sub);
	...
	switch(cmd) {
		case TFA98XX_SWITCH_FIRMWARE_L:
		{
			...
			if (tfa98xx_pri_sub) {
				ret = tfa98xx_ioctrl_probe(tfa98xx_pri_sub);
				if(ret < 0) {
					dev_err(tfa98xx_pri_sub->dev, "Failed to initialize Primary TFA98XX Ampilier: %d", ret);
					goto exit;
				}
			}
			if (tfa98xx_sec_sub) {
				ret = tfa98xx_ioctrl_probe(tfa98xx_sec_sub);
				if(ret < 0) {
					dev_err(tfa98xx_sec_sub->dev, "Failed to initialize Primary TFA98XX Ampilier: %d", ret);
					goto exit;
				}
			}
			break;
		...
		case TFA98XX_SWITCH_CONFIG_L:
			...
			if ((selection & CHAN_SEC_L) && tfa98xx_pri_sub) {
				if (tfa98xx_pri_sub->tfa->is_probus_device) {
					mutex_lock(&tfa98xx_mutex);
					algo_id = (value >> 4) & 0xf;
					mutex_unlock(&tfa98xx_mutex);
					tfa98xx_ioctrl_set_profile(tfa98xx_pri_sub, value & 0xf);
				} else {
					tfa98xx_ioctrl_set_profile(tfa98xx_pri_sub, (value >> 4) & 0xf);
				}
			}
#ifdef CONFIG_ARCH_SPRD
			...
			if (selection & CHAN_PRI_L)
				tfa98xx_ioctrl_mute(tfa98xx_pri, 0);
			if ((selection & CHAN_SEC_L) && (tfa98xx_pri_sub))
				tfa98xx_ioctrl_mute(tfa98xx_pri_sub, 0);
#endif
		...
		case TFA98XX_POWER_OFF_L:
			...
			if ((selection & CHAN_SEC_L) && (tfa98xx_pri_sub))
				tfa98xx_ioctrl_mute(tfa98xx_pri_sub, 1);
			break;	 
		...
		case TFA98XX_SEC_GET_CALIB_STATE_L:
			if ((tfa98xx_pri_sub != NULL) && (tfa98xx_pri_sub->dsp_fw_state == TFA98XX_DSP_FW_OK)) {
				mutex_lock(&tfa98xx_pri_sub->dsp_lock);
				if(tfa_get_bf(tfa98xx_pri_sub->tfa, TFA2_BF_MTPOTC) &&
				   tfa_get_bf(tfa98xx_pri_sub->tfa, TFA2_BF_MTPEX)) {
					value = 1; //default calibration has been set
				} else {
					value = 0; //default calibration has never been set
				}
				mutex_unlock(&tfa98xx_pri_sub->dsp_lock);
			}
			else { /*tfa98xx device is not ready*/
				value = 1;
			}
		...
		case TFA98XX_SEC_SET_RE25C_L:
			...
			if ((tfa98xx_pri_sub != NULL) && (tfa98xx_pri_sub->dsp_fw_state == TFA98XX_DSP_FW_OK)) {
				ret = tfa98xx_set_default_calib(tfa98xx_pri_sub, value);
				if(ret != 0) {
					dev_err(tfa98xx_pri->dev, "Failed to set default calibration value: %d", ret);
				}
			}
		...
		case TFA98XX_SWITCH_FIRMWARE_R:
			if ((tfa98xx_device_count >= 2) && (tfa98xx_sec == NULL) && (tfa98xx_sec_sub == NULL))
				return -2;
			...
			if (tfa98xx_sec_sub) {
				ret = tfa98xx_ioctrl_probe(tfa98xx_sec_sub);
				if(ret < 0) {
					dev_err(tfa98xx_sec_sub->dev, "Failed to initialize Secondary sub TFA98XX Ampilier: %d", ret);
					goto exit;
				}
			}
			break;
		case TFA98XX_SWITCH_CONFIG_R:
			...
			if ((selection & CHAN_PRI_R) && tfa98xx_sec) {
			...
			if ((selection & CHAN_SEC_R) && tfa98xx_sec_sub) {
				if (tfa98xx_sec_sub->tfa->is_probus_device) {
					tfa98xx_ioctrl_set_profile(tfa98xx_sec_sub, value & 0xf);
				} else {
					tfa98xx_ioctrl_set_profile(tfa98xx_sec_sub, (value >> 4) & 0xf);
				}
				tfa98xx_ioctrl_mute(tfa98xx_sec_sub, 0);
			}
			break;	
		case TFA98XX_POWER_OFF_R:
			...
			selection = (value >> 8) & 0xff;
			if (selection & CHAN_PRI_R)
				tfa98xx_ioctrl_mute(tfa98xx_sec, 1);
			if ((selection & CHAN_SEC_R) && (tfa98xx_sec_sub))
				tfa98xx_ioctrl_mute(tfa98xx_sec_sub, 1);
			break;	 
		...
		case TFA98XX_SEC_GET_CALIB_STATE_R:
			if ((tfa98xx_sec_sub != NULL) && (tfa98xx_sec_sub->dsp_fw_state == TFA98XX_DSP_FW_OK)) {
				mutex_lock(&tfa98xx_sec_sub->dsp_lock);
				if(tfa_get_bf(tfa98xx_sec_sub->tfa, TFA2_BF_MTPOTC) &&
				   tfa_get_bf(tfa98xx_sec_sub->tfa, TFA2_BF_MTPEX)) {
					value = 1; //default calibration has been set
				} else {
					value = 0; //default calibration has never been set
				}
				mutex_unlock(&tfa98xx_sec_sub->dsp_lock);
			}
			else { /*tfa98xx device is not ready*/
				value = 1;
			}
		...
		case TFA98XX_SEC_SET_RE25C_R:
			...
			if ((tfa98xx_sec_sub != NULL) && (tfa98xx_sec_sub->dsp_fw_state == TFA98XX_DSP_FW_OK)) {
				ret = tfa98xx_set_default_calib(tfa98xx_sec_sub, value);
				if(ret != 0) {
					dev_err(tfa98xx_pri->dev, "Failed to set default calibration value: %d", ret);
				}
			}
		...
		case TFA98XX_DO_REGDUMP:
			...
			if (tfa98xx_pri_sub != NULL) {
				ret = tfa98xx_do_regdump(tfa98xx_pri_sub);
				if(ret != 0) {
					pr_err("tfa98xx_do_regdump sec failed with return: %d", ret);
				}
			}
			if (tfa98xx_sec_sub != NULL) {
				ret = tfa98xx_do_regdump(tfa98xx_sec_sub);
				if(ret != 0) {
					pr_err("tfa98xx_do_regdump sec failed with return: %d", ret);
				}
			}
					 ...
					 loff_t *ppos)
{
	...
	size_t size_per_device = count / TFACONT_MAXDEVS;	 /* calculate buffer size per device. */
	...
	if (tfa98xx_pri->tfa->is_probus_device) {
		error = tfa98xx_recv_data_from_dsp(buffer + TFA_WORD_LEN * 3, count); /* skip 9 bytes for non-dsp solution. */
	} else {
		error = tfa_dsp_msg_read(tfa98xx_pri->tfa, size_per_device, &buffer[count]);
	}
	...
	if (tfa98xx_sec && tfa98xx_sec->tfa->is_probus_device == 0) {
		mutex_lock(&tfa98xx_sec->dsp_lock);
		error = tfa_dsp_msg_read(tfa98xx_sec->tfa, size_per_device, &buffer[count + size_per_device]);
		mutex_unlock(&tfa98xx_sec->dsp_lock);
	}
	...
	/*Read 3rd channel if available*/
	if (tfa98xx_pri_sub && tfa98xx_pri_sub->tfa->is_probus_device == 0) {
		mutex_lock(&tfa98xx_pri_sub->dsp_lock);
		error = tfa_dsp_msg_read(tfa98xx_pri_sub->tfa, size_per_device, &buffer[count + size_per_device * 2]);
		mutex_unlock(&tfa98xx_pri_sub->dsp_lock);
	}

	if (error != Tfa98xx_Error_Ok) {
		pr_debug("[0x%x] dsp_msg_read error: %d\n", tfa98xx_pri_sub->i2c->addr, error);
		kfree(buffer);
		return -EFAULT;
	}
	/*Read 4th channel if available*/
	if (tfa98xx_sec_sub && tfa98xx_sec_sub->tfa->is_probus_device == 0) {
		mutex_lock(&tfa98xx_sec_sub->dsp_lock);
		error = tfa_dsp_msg_read(tfa98xx_sec_sub->tfa, size_per_device, &buffer[count + size_per_device * 3]);
		mutex_unlock(&tfa98xx_sec_sub->dsp_lock);
	}

	if (error != Tfa98xx_Error_Ok) {
		pr_debug("[0x%x] dsp_msg_read error: %d\n", tfa98xx_sec_sub->i2c->addr, error);
		kfree(buffer);
		return -EFAULT;
	}
	...
	if (tfa98xx_pri->tfa->is_probus_device == 0) {
		int dev;
		for (i = 0; i < count /(TFACONT_MAXDEVS*TFA_WORD_LEN); i++) { /* calculate how many words. */
			for (dev = 0; dev < TFACONT_MAXDEVS; dev++) {
				buffer[i * TFACONT_MAXDEVS * TFA_WORD_LEN + dev * TFA_WORD_LEN + 0] = buffer[count + size_per_device * dev + i * TFA_WORD_LEN + 0];
				buffer[i * TFACONT_MAXDEVS * TFA_WORD_LEN + dev * TFA_WORD_LEN + 1] = buffer[count + size_per_device * dev + i * TFA_WORD_LEN + 1];
				buffer[i * TFACONT_MAXDEVS * TFA_WORD_LEN + dev * TFA_WORD_LEN + 2] = buffer[count + size_per_device * dev + i * TFA_WORD_LEN + 2];
			}
		}
	}
					 ...
					 size_t count, loff_t *ppos)
{
	...
	if (tfa98xx_pri_sub && tfa98xx_pri_sub->tfa->is_probus_device == 0) {
		...
		error = tfa_dsp_msg(tfa98xx_pri_sub->tfa, msg_file->size, msg_file->data);
		if (error != Tfa98xx_Error_Ok) {
			pr_debug("[0x%x] dsp_msg error: %d\n", tfa98xx_pri_sub->i2c->addr, error);
			err = -EIO;
		}
	...
	if (tfa98xx_sec_sub && tfa98xx_sec_sub->tfa->is_probus_device == 0) {
		...
		error = tfa_dsp_msg(tfa98xx_sec_sub->tfa, msg_file->size, msg_file->data);
		if (error != Tfa98xx_Error_Ok) {
			pr_debug("[0x%x] dsp_msg error: %d\n", tfa98xx_sec_sub->i2c->addr, error);
			err = -EIO;
		}
	...
	const struct i2c_device_id *id)
{
	...
	if (ret)
		...
		tfa98xx_set_vendor_info(VENDOR_GOODIX, "01-03");
#endif /*ENABLE_IOCTRL_INTERFACE*/
