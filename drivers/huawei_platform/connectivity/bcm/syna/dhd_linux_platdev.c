...
extern int dhd_wlan_power(int onoff);
...
void* wifi_platform_prealloc(wifi_adapter_info_t *adapter, int section, unsigned long size)
{
	...
	if (!adapter || !adapter->wifi_plat_data) {
		alloc_ptr = kmalloc(size, GFP_KERNEL);
		printk("%s: enter %d section = %d prealloc error #################\n", __func__, __LINE__, section);
		return alloc_ptr;
	}

	plat_data = adapter->wifi_plat_data;
	if (plat_data->mem_prealloc) {
		printk("try to get pre alloc memory section %d, size %d\n", section, size);
...
static ssize_t restore_wifi_factory_dbg(struct device *dev, struct device_attribute *attr,
    ...
    if (sscanf(buf, "%d\n", &value) == 1) {
        if (HW_FAC_GPIO_VAL_LOW == value) {
            ret = dhd_wlan_power(0);
        } else if (HW_FAC_GPIO_VAL_HIG == value) {
            ret = dhd_wlan_power(1);
        } else {
			DHD_ERROR(("%s: restore_wifi_factory_dbg: value: %d\n", __FUNCTION__, value));
        }
    }
    return ((ret >= 0) ? size: -1);
}
...
// modify for compaibility
	...
	if (ret) {
		DHD_ERROR(("%s: platform_device_register failed, ret:%d.\n", __FUNCTION__, ret));
		return ret;
	}

	ret = sysfs_create_group(&bcm_wifi_device.dev.kobj, &wifi_state);
	if (ret) {
		DHD_ERROR(("%s: sysfs_create_group failed(%d)\n", __FUNCTION__, ret));
		return ret;
	}
...
static int dhd_wifi_platform_load_sdio(void)
{
	...
	for (i = 0; i < dhd_wifi_platdata->num_adapters; i++) {
		...
		do {
			...
			err = down_timeout(&dhd_chipup_sem, msecs_to_jiffies(POWERUP_WAIT_MS));
