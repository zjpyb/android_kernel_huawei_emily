...
#ifdef CUSTOMER_HW4
/* Enforce CONFIG_BCMDHD_PREALLOC_MEMDUMP if CONFIG_BROADCOM_WIFI_RESERVED_MEM
 * is defined.
 */
...
*dhd_wlan_mem_prealloc(int section, unsigned long size)
{
	int i;
	int j;
	printk("%s: get memory section=%d, size=%d\n", __func__, section, size);
	if (section == PREALLOC_WLAN_SEC_NUM) {
		if (!wlan_static_skb[0]) {
#if !defined(BCMPCIE)
	...
	if (wlan_mem_array[section].size < size) {
		pr_err("request wlan_mem_array[%d], size(%d) > static size(%d)\n",
				i, size, wlan_mem_array[i].size);
		return NULL;
	}

	if (!wlan_mem_array[section].mem_ptr) {
		wlan_mem_array[section].mem_ptr =
