...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	if (try_cnt > CUSTOM_MAX_KSO_ATTEMPTS)  {
		DHD_ERROR(("%s> op:%s, ERROR: try_cnt:%d, rd_val:%x, ERR:%x \n",
			__FUNCTION__, (on ? "KSO_SET" : "KSO_CLR"), try_cnt, rd_val, err));
		bus->dhd->hang_reason = HANG_REASON_BUS_DOWN;
...
#if defined(DHD_SDIO_DYN_TXGLOM_SIZE)
int dhd_set_sdio_txglom(dhd_pub_t *dhdp, int val)
{
	dhd_bus_t *bus = dhdp->bus;
	if (bus)  {
		if (val == 0) {
			bus->txglomsize = SDPCM_MAXGLOM_SIZE;
		} else if (val <= SDPCM_MAXGLOM_SIZE) {
			bus->txglomsize = val;
		}
	}
...
#endif /* DHD_SDIO_DYN_TXGLOM_SIZE */
...
exit:
	...
	if (bus->dhd->bus_ts.tv_sec == 0 && bus->dhd->bus_ts.tv_nsec == 0) {
		osl_do_gettimeofday(&bus->dhd->bus_ts);
	} else {
		...
		diff_ms = osl_do_gettimediff(&cur_ts, &bus->dhd->bus_ts)/1000;
		if (diff_ms >= 1000) {
			...
			memcpy(&bus->dhd->bus_ts, &cur_ts, sizeof(struct osl_timespec));
			#define DHD_POLLING_THROUGHPUT_THRESHOLD (100) // 100Mbps
			if((tx_tput+rx_tput) > DHD_POLLING_THROUGHPUT_THRESHOLD) {
				g_dhd_escan_on = FALSE;
				hw_wifi_set_cpu_performance(1);
			} else {
				g_dhd_escan_on = TRUE;
				hw_wifi_set_cpu_performance(0);
			}
		}
	}
...
#define TEL_HUAWEI_NV_WINVRAM_LENGTH             104
	...
	if (!ret)
	{
		...
		DHD_ERROR(("%s: hw_load_Txcalibrate_data!\n", __func__));
...
#define TEL_HUAWEI_NV_WIRXNVRAM_LENGTH             104
	...
	{
		strncpy(calbufp, info.nv_data, size);
		DHD_ERROR(("%s: hw_load_Rxcalibrate_data!\n", __func__));
...
#define TEL_HUAWEI_NV_WITXCCK_LENGTH             104
	...
	{
		strncpy(calbufp, info.nv_data, size);
		DHD_ERROR(("%s: hw_load_MimoTxcckcalibrate_data!\n", __func__));
		return strlen(calbufp);
...
#define TEL_HUAWEI_NV_WITXC1_LENGTH             104
	...
	{
		strncpy(calbufp, info.nv_data, size);
		DHD_ERROR(("%s: hw_load_MimoTxc1calibrate_data!\n", __func__));
		return strlen(calbufp);
	}
	DHD_ERROR(("%s: No data\n", __func__));
	return 0;
...
#define TEL_HUAWEI_NV_WITXBW0_LENGTH             104
	...
	{
		strncpy(calbufp, info.nv_data, size);
		DHD_ERROR(("%s: hw_load_MimoTxbw0calibrate_data!\n", __func__));
		return strlen(calbufp);
	}
	DHD_ERROR(("%s: No data\n", __func__));
	return 0;
...
#define TEL_HUAWEI_NV_WITXBW1_LENGTH             104
	...
	{
		strncpy(calbufp, info.nv_data, size);
		DHD_ERROR(("%s: hw_load_MimoTxbw1calibrate_data!\n", __func__));
		return strlen(calbufp);
	}
	DHD_ERROR(("%s: No data\n", __func__));
	return 0;
