...
#endif /* WL_SUPPORT_AUTO_CHANNEL */
#define CUSTOMER_HW4_PRIVATE_CMD
...
#define CMD_SARLIMIT_TX_CONTROL		"SET_TX_POWER"
...
{
	int err = BCME_ERROR;
	int mode = bcm_atoi(string_num) - 1000;

	/* As Huawei specific and their requirement,
	 * the mode set as the following form.
	 *  0 : SAR disable
	 *  1 : HEAD SAR enabled
	 *  2 : GRIP SAR enabled
	 */
	if ((mode < SAR_DISABLE ) || (mode > SAR_GRIP)) {
		DHD_ERROR(("%s: Request for Unsupported:%d\n", __FUNCTION__, bcm_atoi(string_num)));
		err = BCME_RANGE;
		goto error;
	}

	err = wldev_iovar_setint(dev, "sar_enable", mode);
	if (unlikely(err)) {
...
#ifdef HUAWEI_ANDROID_EXTENSION
#if defined(HW_SOFTAP_MANAGEMENT)
...
#endif /* HW_SOFTAP_MANAGEMENT */
#endif /* HUAWEI_ANDROID_EXTENSION */
...
wl_handle_private_cmd(struct net_device *net, char *command, u32 cmd_len)
{
	...
	/* CMD_AMPDU_MPDU */
		...
		int skip = strlen(CMD_SARLIMIT_TX_CONTROL) + 3; /* "SET_TX_POWER 0 1000"  skip " 0 "*/
