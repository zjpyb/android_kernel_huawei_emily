...
extern int dhd_set_sdio_txglom(dhd_pub_t *dhdp, int val);
...
int wl_features_set(u8 *array, uint8 len, u32 ftidx)
{
	...
	*ft_byte |= BIT(ftidx % 8u);
	...
	0))) || defined(CUSTOMER_HW6) || defined(WIPHY_DYNAMIC_UPDATE)
...
#endif /* WL_SELF_MANAGED_REGDOM && KERNEL >= 4.0 || CUSTOMER_HW6 || WIPHY_DYNAMIC_UPDATE */
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_JOIN || event == WLC_E_START ||
		...
		if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
			...
			if (!wl_get_drv_status(cfg, CONNECTED, bcmcfg_to_prmry_ndev(cfg)) && ((event == WLC_E_JOIN) && !(status & 0x80))) {
				wl_wlfc_enable(cfg, false);
			}
			wl_link_up(cfg);
			wl_get_assoc_ies(cfg, ndev);
...
s32 wl_cfg80211_up(struct net_device *net)
{
	...
	if(dhd->during_roaming && cfg) {
		wl_roam_timeout((unsigned long)cfg);
	}
#if defined(BCMSUP_4WAY_HANDSHAKE)
