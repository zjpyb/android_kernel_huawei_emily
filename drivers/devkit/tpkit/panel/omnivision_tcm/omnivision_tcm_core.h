...
#define LOG_X(func, dev, log, ...) \
	func(dev, "%s info: " log, __func__, ##__VA_ARGS__)

#define LOG_Y(func, dev, log, ...) \
	func(dev, "%s error: (line %d) " log, __func__, __LINE__, ##__VA_ARGS__)

#define LOGD(dev, log, ...) LOG_X(dev_dbg, dev, log, ##__VA_ARGS__)
#define LOGI(dev, log, ...) LOG_X(dev_info, dev, log, ##__VA_ARGS__)
#define LOGN(dev, log, ...) LOG_X(dev_notice, dev, log, ##__VA_ARGS__)
#define LOGW(dev, log, ...) LOG_Y(dev_warn, dev, log, ##__VA_ARGS__)
#define LOGE(dev, log, ...) LOG_Y(dev_err, dev, log, ##__VA_ARGS__)

#define OVT_LOG_INFO(fmt, arg...) printk("TS_KIT [%s] "fmt"\n", __func__, ##arg)
#define OVT_LOG_ERR(fmt, arg...) printk("TS_KIT [%s] "fmt"\n", __func__, ##arg)
#define OVT_LOG_DEBUG(fmt, arg...) printk("TS_KIT [%s] "fmt"\n", __func__, ##arg)
...
extern int ovt_testing_init(struct ovt_tcm_hcd *tcm_hcd);
...
static inline int secure_cpydata(unsigned char *dest, unsigned int dest_size,
		...
		struct ovt_tcm_buffer *buffer, unsigned int size)
{
	...
	if (size > buffer->buf_size) {
		...
		retval = secure_cpydata(buffer->buf,
				size,
				temp,
				buffer->buf_size,
				buffer->buf_size);
