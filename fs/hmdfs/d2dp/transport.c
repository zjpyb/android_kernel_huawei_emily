...
static bool prepare_ack(struct d2d_protocol *proto)
{
	...
	/*
	 * The ACK is ready when any of the following is true:
	 * - last ACK id has been changed from the last time
	 * - there are some SACKS pairs generated by RX buffer
	 * - we need to set or flush flowcontrol SUSPEND flag
	 * - the ACK is forced
	 *
	 * Otherwise, no need to send ACK packet and `ready` flag is not set
	 */
...
bool ack_timer_action(void *arg)
{
	...
	t_start = ktime_get();
	ready = prepare_ack(proto);
	t_end = ktime_get();

	if (ready)
		wake_up_interruptible_all(&tx->wq_data);

	trace_d2dp_ack_timer(ktime_sub(t_end, t_start), ready);
...
bool rto_timer_action(void *arg)
{
	...
	trace_d2dp_rto_timer(ktime_sub(t_end, t_start), self->retry_num);
...
static bool tx_wait_cond_lock(struct d2d_protocol *proto, int *reason)
{
	...
	spin_unlock(&tx->lock);
	atomic_inc(&proto->stats.tx_wait_switches);
	return wakeup;
}
				       ...
				       struct d2d_header *header)
{
	...
	atomic_sub(freed, &proto->stats.tx_buf_bytes);
			    ...
			    const void *data)
{
	...
	if (res == RX_BUF_PUT_OK)
		/* we have to add value before unlocking to get the ordering */
		atomic_add(packet.len, &proto->stats.rx_buf_bytes);
